...
func (ss *ScreenStack) update(s tea.Model) { (*ss)[len(*ss)-1] = s }
...
func (a *ApplicationTUI) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case YesNoResponse:
		if _, ok := cur.(YesNoPrompt); !ok {
			panic("sending of YesNoResponse should be YesNoPrompt")
		}
		a.screens.pop()
		return a, func() tea.Msg { return a.pending(msg.response) }
	}
	if prev == a.screens.cur() {
		a.screens.update(cur)
	}
	return a, cmd
}

func (a *ApplicationTUI) View() string { return a.screens.cur().View() }
